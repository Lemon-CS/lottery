...
// 当前时间的时间戳
func NowUnix() int {
	return int(time.Now().In(config.SysTimeLocation).Unix())
}

// 将unix时间戳格式化为yyyymmdd H:i:s格式字符串
func FormatFromUnixTime(t int64) string {
	if t > 0 {
		return time.Unix(t, 0).Format(config.SysTimeform)
	} else {
		return time.Now().Format(config.SysTimeform)
	}
}
...
func FormatFromUnixTimeShort(t int64) string {
	if t > 0 {
		return time.Unix(t, 0).Format(config.SysTimeformShort)
	} else {
		return time.Now().Format(config.SysTimeformShort)
	}
}

// 将字符串转成时间
func ParseTime(str string) (time.Time, error) {
	return time.ParseInLocation(config.SysTimeform, str, config.SysTimeLocation)
}
...
func CreateSign(str string) string {
	str = string(config.SignSecret) + str
...
func NextDayDuration() time.Duration {
	...
	next := time.Date(year, month, day, 0, 0, 0, 0, config.SysTimeLocation)
